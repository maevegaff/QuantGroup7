...
#gone through to line 182
#make the code messier by adding in some of his techniques 
##Variables for GUI- document, number of iterations, risk free rate for sharpe ratio 
...
from pathlib import Path #used to check if the file path is correct and the file can be accessed, and locating files within the script
import seaborn as sns #package used to help generate heat map graphics
from itertools import combinations #used to generate combinations, for the monte carlo done in the section 
...
# Any Nan values are dropped a fail safe interpolation command is written as a follow up 
...
# Removal is selected as the method for simplicity data cleaning as there is high number of data points so statistical signicifcance should not be sacrificed 
# Additionally the later use of a monte carlo may extrapolate any estimates 
prices_df = prices_df.dropna()
print(f"\nData shape after dropping NaN rows: {prices_df.shape}")

# Interpolate any remaining missing values (just in case the is any unaccounted values that are not in the first column drop)
...
# Preliminary asset analysis of complete data frame

# Calculate Daily Returns
returns_df = prices_df.pct_change().dropna()
print(f"\nDaily Simple Returns DataFrame shape: {returns_df.shape}")
...
# Generate All Valid 5-Asset Portfolio Combinations- debug print
...
valid_portfolios = []
num_simulations = 100  # Number of random weight simulations per combination (selected around data needed and run time)
...
####outer loop is used to loop five different asset for combination
####it also calls for a printed progress report during the simulation 
for combo_idx, selected_assets in enumerate(valid_combinations):
    ...
    # Track the best of each metric for further analysis
    # Reset at the iteration of the outer loop (helps due to high sample numbers)
    # Metric is stored based on its rank from previous values starting from a n
    ...
    ##inner loop generates weight allocations
    for sim in range(num_simulations):
        ...
        ####the weight,  variance and standard deviation is calculated for the randomly generated portfolio 
        ...
        ####not sure what risk free rate to add can be changable in GUI
        sharpe_ratio = portfolio_return / portfolio_std_dev if portfolio_std_dev > 0 else 0
        
        # Keep track of the best weights for this combination (similar to previous metric tracking code))
    ...
    # Stores the tracked variables as lists
...
print(top_10[['Assets', 'Weights', 'Annual_Return', 'Annual_Risk', 'Sharpe_Ratio', 'Avg_Correlation', 'Max_Weight']])

# Save top 10 portfolios to a CSV file
top_10_export = top_10.copy()
top_10_export['Weights_Str'] = top_10_export['Weights'].apply(lambda w: ', '.join([f'{x:.4f}' for x in w]))
top_10_export['Assets_Str'] = top_10_export['Assets'].apply(lambda x: ', '.join(x))
top_10_export = top_10_export[['Assets_Str', 'Weights_Str', 'Annual_Return', 'Annual_Risk', 
                               'Sharpe_Ratio', 'Avg_Correlation', 'Max_Weight']]
top_10_export.columns = ['Assets', 'Weights', 'Annual_Return', 'Annual_Risk', 
                         'Sharpe_Ratio', 'Avg_Correlation', 'Max_Weight']
top_10_export.to_csv('top_10_portfolios.csv', index=False)
print("\nTop 10 portfolios saved to 'top_10_portfolios.csv'")
...
# Plot 1: Scatter plot of Efficient Frontier from sim
...
# Analyze which assets appear most frequently in top 5 portfolios
...
# Save asset frequency results to a CSV file
# To be used for momentum analysis 
asset_frequency_df = pd.DataFrame(
    [{'Asset': asset, 'Frequency': count, 'Asset_Class': get_asset_class(asset, asset_classes)} 
     for asset, count in sorted(asset_frequency.items(), key=lambda x: x[1], reverse=True)]
)
asset_frequency_df.to_csv('asset_frequency_top_5_pct.csv', index=False)
print("\nAsset frequency results saved to 'asset_frequency_top_5_pct.csv'")
...
for asset, count in sorted(asset_frequency.items(), key=lambda x: x[1], reverse=True):
    ...
    # Create an equal-weighted portfolio for the top 5 assets
    top_5_assets = asset_frequency_df.nlargest(5, 'Frequency')['Asset'].tolist()
    equal_weights = np.full(len(top_5_assets), 1 / len(top_5_assets))  # Equal weights

    # Calculate portfolio return
    top_5_returns = expected_annual_returns[top_5_assets]
    equal_weighted_return = np.dot(top_5_returns, equal_weights)

    # Save the result to a CSV file
    equal_weighted_portfolio = pd.DataFrame({
        'Assets': [', '.join(top_5_assets)],
        'Weights': [', '.join([f'{w:.4f}' for w in equal_weights])],
        'Annual_Return': [equal_weighted_return]
    })
    equal_weighted_portfolio.to_csv('equal_weighted_top_5_portfolio.csv', index=False)
    print("\nEqual-weighted portfolio for top 5 assets saved to 'equal_weighted_top_5_portfolio.csv'")
